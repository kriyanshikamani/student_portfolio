...
public partial class faculty_resultPage : System.Web.UI.Page
{
    ...
    protected void LoadSubjects()
    {
        ...
        while (reader.Read())
        {
            ...
            txtInternalMarks.ID = "txtInternalMarks_" + i; // Unique ID for each textbox
    ...
    {
        for (int i = 1; i < tableSubjects.Rows.Count; i++)
        {
            // Retrieve the subject name, internal marks, and exam marks from the corresponding textboxes
            string subject = tableSubjects.Rows[i].Cells[0].Text;
            TextBox txtInternalMarks = (TextBox)tableSubjects.Rows[i].Cells[1].Controls[0];
            TextBox txtExamMarks = (TextBox)tableSubjects.Rows[i].Cells[2].Controls[0];

            // Insert the data into the Result table

            Response.Write(subject);
            Response.Write(txtInternalMarks.Text);
            Response.Write(txtExamMarks.Text);
